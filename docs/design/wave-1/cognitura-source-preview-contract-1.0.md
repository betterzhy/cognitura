...
PreIdentityValidationErrors = UNSUPPORTED_MEDIA_TYPE,EMPTY_SOURCE_FILE,SOURCE_SIZE_LIMIT,SOURCE_HASH_MISMATCH
```
...
API_ERROR: EMPTY_SOURCE_FILE -> 422,false,IDENTITIES_NULL
API_ERROR: SOURCE_HASH_MISMATCH -> 422,false,IDENTITIES_NULL
...
D01 的四个 pre-registration 错误始终使用 `IDENTITIES_NULL`，因为正式来源尚未
创建；`SOURCE_SIZE_LIMIT` 用 413、`UNSUPPORTED_MEDIA_TYPE` 用 415，空文件与
声明 hash 不匹配用 422。只有注册后的 DOCX 安全/格式拒绝可以使用
`CREATED_IDENTITIES_ONLY` 并返回已创建的 SourceDocument identity。
