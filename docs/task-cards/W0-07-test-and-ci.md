...
将全部 Wave 0 校验统一纳入本地可重复入口和 CI，使来源、Schema、Golden Case、
页面契约、server 与 web 构建在每次提交上自动执行。
...
- [ ] 先编写 CI 等价测试，故意破坏临时 source hash 并要求总入口失败。
- [ ] 运行测试，确认统一入口或 workflow 不存在时失败。
- [ ] 创建 `scripts/verify-wave0`，按固定顺序运行来源、任务卡、Schema、Golden Case、
  UI、server 和 web 校验。
- [ ] 创建 CI jobs，使用 lockfile 缓存键且不使用浮动依赖。
- [ ] 配置 PostgreSQL 18 临时测试容器，不连接正式数据库。
- [ ] 验证 CI 不修改 `raw/`、不访问 Redis 遗留链接、不执行正式数据库写入。
- [ ] 运行本地完整入口并核对 CI 结果、日志和失败传播。
- [ ] 更新测试策略、任务卡状态和 Gate，并形成独立提交。
