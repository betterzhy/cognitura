...
```bash
git add server/src/main/java/io/cognitura/source/api/command \
  server/src/test/java/io/cognitura/source/api/command
git commit -m "feat: add source processing commands"
```

固定提交由新的 `deep_reviewer` 审查；零发现 GO 前不得释放后继。
