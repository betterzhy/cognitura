...
- [x] **Step 2: Commit the repair candidate locally without `.idea/`.**
- [x] **Step 3: Run an independent `gpt-5.6-sol/high` general review on the exact commit.**
...
- [x] **Step 4: If and only if the general review is zero-finding, run a separate `gpt-5.6-sol/high` final Gate.**
- [x] **Step 5: Synchronize W1-DG5 and all status projections to the actual result, rerun all verification and create a local closure commit.**
- [x] **Step 6: Confirm no remote push occurred and report exact SHAs, Gate evidence and remaining authorization boundary.**
